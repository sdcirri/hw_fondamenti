...
def ex(file_dati, file_png, spaziatura):
    ...
    for i in p:                                                                 # Per ogni isolato
        pw.append(sum([j[0] for j in i]) + spaziatura * (len(i) - 1))           # Stima la larghezza assumendo che sia il
                                                                                #  più largo
    maxw = max(pw)
    ...
    for i in p:                                                                 # Per ogni isolato
        maxh.append(max([j[1] for j in i]))                                     # Trova il lato N-S massimo e aggiungilo
                                                                                #  alla lista
    h = sum(maxh) + spaziatura * (len(p) + 1)                                   # Calcola l'altezza della pianta finale
    ...
    for i, ix in enumerate(p):                                                  # Per ogni isolato
        ...
        for j in ix:                                                            # Per ogni palazzo dell'isolato
            deltah = (maxh[i] - j[1]) // 2                                      # Calcola la dimensione dei due giardini
            if len(ix) == 1:                                                    # Se l'isolato contiene un solo palazzo
                drawRect(img, x + deltaw // 2, y + deltah, j[0], j[1], j[2])    # Assumi due strade secondarie ai lati
            else:                                                               # Altrimenti
                drawRect(img, x, y + deltah, j[0], j[1], j[2])                  # Disegna il palazzo da dove ti trovi
                x += j[0] + deltaw                                              # E spostati di (lungh. palazzo + lungh. strada)
    ...
    return w, h                                                                 # Ritorna le dimensioni della pianta


if __name__ == '__main__':
    # inserisci qui i tuoi test personali per debuggare
    print(ex('matrices/example.txt', 'test_boh.png', 42))
